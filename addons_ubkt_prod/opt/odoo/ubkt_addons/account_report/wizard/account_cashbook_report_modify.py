...
class AccountCashBookReport(models.TransientModel):
    _name = "account.cashbook.modify.report"
    _description = "Cash Book modify Report"
    ...
    account_ids = fields.Many2many('account.account', 'account_account_cashbook_modify_report', 'report_line_id',
    ...
    def check_report(self):
        ...
        if self.initial_balance is True:
            return self.env.ref(
                'account_report.action_report_cash_book_modify_initial_balance').report_action(self,
                                                                         data=data)
        else:
            return self.env.ref(
                'account_report.action_report_cash_book_modify').report_action(self,
                                                                               data=data)
