...
class ReportCashBook(models.AbstractModel):
    _name = 'report.account_report.report_cashbook_modify'
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        sql = ('''SELECT l.id AS lid, l.journal_id as journal_id, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, j.name AS journal_name, l.payment_id as lpayment, l.currency_id, l.amount_currency, l.ref AS lref, ap.paper_payment_code as lpaper_payment_code, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
                        ...
                        JOIN account_journal j ON (l.journal_id=j.id)\
                        JOIN account_account acc ON (l.account_id = acc.id) \
                        WHERE l.account_id IN %s ''' + filters + ''' GROUP BY l.id, l.account_id, l.journal_id, l.date, j.code, j.name , l.currency_id, l.amount_currency, l.ref, ap.paper_payment_code, l.name, m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if not accounts:
            ...
            for journal in journals:
                accounts.append(journal.payment_credit_account_id.id)
            accounts = self.env['account.account'].search([('id', 'in', accounts)])
        record, l_total = self.with_context(data['form'].get('comparison_context', {}))._get_account_move_entry(accounts, init_balance, sortby, display_account)
        return {
            'doc_ids': docids,
            'doc_model': model,
            'data': data['form'],
